...
| **Extreme-tile P4 pilot** | P4 Stage-A proxy ceiling on one 2018 heat/drought tile (`32UQC`, 346 of 348 non-overlapping cubes), same screen/folds/estimators/`weather_full8` as Screened P2/P4 so the tables sit side by side. A different TILE, not a rerun of 32UNU — the first result outside `data/scaled_32UNU/` | `scripts/run_p4_extreme.py`, `probes/p4_ceiling.py` | `data/scaled_32UQC/p4_extreme_results.csv`, `data/scaled_32UQC/p4_extreme_run.log`, `data/scaled_32UQC/raw/`, `notebooks/runs/2026-08-17_p4_extreme_32UQC_346cubes.txt` | 2026-08-17 | `## 2026-08-17: Extreme-tile P4 pilot...` |
| **Extreme-tile P3** | The nine-view forecastability probe on the same 2018 heat/drought tile the P4 pilot measured (`32UQC`, 342 of 348 cubes after four separate exclusions), same four Tier-1 corrections and screen, so it reads directly against the 115-cube 32UNU Tier-1 table. Encoding on Colab (T4, python >= 3.10 -- the dev venv cannot build DINOv2 at all); fitting on CPU. A SUBSET table: `fold_mode=loco` dropped by the runtime valve, all nine encoder views kept | `notebooks/phase1_10_extreme_encoding.ipynb`, `scripts/run_p3_extreme.py`, `probes/p3_forecast.py`, `probes/p3_triggers.py` | `data/scaled_32UQC/p3_extreme_subset_results.csv`, `data/scaled_32UQC/p3_extreme_predictions.csv.gz`, `data/scaled_32UQC/p3_extreme_triggers.csv`, `data/scaled_32UQC/{embeddings,embeddings_cir,masks}/`, `notebooks/runs/2026-08-19_p3_extreme_32UQC_342cubes.txt` | 2026-08-19 | `## 2026-08-19: Extreme-tile P3 on 32UQC...` |
| **Extreme-tile P3 `loco` top-up** | The 308 `fold_mode=loco` rows the runtime valve dropped from the local run, computed on TUM's lxhalle (AMD EPYC 9554P, 64 cores; 116.7 min at 23.0 s/row) and merged back. Completes `Extreme-tile P3` to the full 1540-row grid. No GPU, no code change beyond `--no-predictions` / `--max-cubes`; library versions pinned to the Mac's so the two halves are one experiment | `scripts/run_p3_extreme.py --fold-modes loco --aggregations cube_mean`, `scripts/merge_loco.py`, `docs/SERVER_REPLICATION_STEPS.md` | `data/scaled_32UQC/p3_extreme_results.csv` (the merged 1540-row table), `data/scaled_32UQC/p3_extreme_loco_results.csv`, `notebooks/runs/2026-08-19_p3_extreme_loco_32UQC_342cubes_lxhalle.txt` | 2026-08-19 | `## 2026-08-19 (later): the extreme-tile P3 table is COMPLETE...` |
